	...
	public HapConfig(HapConfig[] final_local_haps){
		...
		for (int p = 0; p < final_local_haps.length; p++) {
			...
			for (int h = 0; h < final_local_haps[p].num_global_hap; h++) {
				String curr_vc = "";
				for (int l = 0; l < this.num_loci; l++) curr_vc += final_local_haps[p].global_haps_string[h][l]; 
				if (!global_ids.contains(curr_vc)) global_ids.add(curr_vc); 
		...
		for (int h = 0; h < this.num_global_hap; h++) {
			...
			for (int l = 0; l < this.num_loci; l++) this.global_haps_string[h][l] = hap_var_comp[l];
